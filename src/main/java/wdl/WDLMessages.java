...
public class WDLMessages {
	...
	public static void chatMessageTranslated(@Nonnull IConfiguration config,
		...
		// 	component = new ChatComponentTranslation(translationKey, args);
		// } else {
	 ...
	 */
	public static void chatMessage(@Nonnull IConfiguration config,
			@Nonnull IWDLMessageType type, @Nonnull IChatComponent message) {
		boolean enabled;
		try {
	...
	@Nonnull
	private static IChatComponent convertEntityToComponent(@Nonnull Entity e) {
	...
	@Nonnull
	private static IChatComponent convertThrowableToComponent(@Nonnull Throwable t) {
